...
public class StudentProfilePanel extends JPanel {
    ...
    private JComboBox<ClassDTO> cbClass = new JComboBox<>();   // chỉ hiển thị
    ...
    private JComboBox<String> cbStatus = new JComboBox<>(new String[]{"ACTIVE","GRADUATED","DROPPED","SUSPENDED"}); // chỉ hiển thị
    ...
    public StudentProfilePanel(UserDTO user) {
        ...
        south.setBackground(new Color(250,252,255));
    ...
    private void loadProfile() {
        try {
            ...
            if (s != null) {
                ...
                cbStatus.setSelectedItem(s.getStatus()); // readonly
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void save() {
        try {
            ...
            s.setClassId(((ClassDTO) cbClass.getSelectedItem()).getClassId()); // giữ nguyên
            ...
            s.setStatus((String) cbStatus.getSelectedItem()); // giữ nguyên

            StudentService svc = RMIConnector.student();
            boolean ok = svc.updateStudent(s);
            if (ok) UIUtils.info(this,"Saved successfully");
